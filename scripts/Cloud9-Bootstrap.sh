...
/usr/bin/envsubst < "1click-hpc/parallelcluster/config.${AWS_REGION_NAME}.sample.yaml" > cluster.config
...
#Create the cluster
/home/ec2-user/.local/bin/pcluster create-cluster --cluster-name "hpc-1click-${CLUSTER_NAME}" --cluster-configuration cluster.config --rollback-on-failure false --wait

#/home/ec2-user/.local/bin/pcluster describe-cluster --cluster-name hpc-1click-PC3-22501 | jq -r .clusterStatus
#CREATE_IN_PROGRESS
