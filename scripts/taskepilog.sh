...
if [[ $compute_instance_type =~ $gpu_instances ]]; then
    /usr/local/bin/docker-compose -f "${monitoring_home}/docker-compose/docker-compose.compute.gpu.yml" down
else
    /usr/local/bin/docker-compose -f "${monitoring_home}/docker-compose/docker-compose.compute.yml" down
fi