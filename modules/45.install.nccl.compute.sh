...
installNCCL() {
    ...
    cd nccl-cuda11.4
    git checkout 'v2.12.12-1'
    make -j src.build CUDA_HOME=/usr/local/cuda-11.4 NVCC_GENCODE='-gencode=arch=compute_70,code=sm_70 -gencode=arch=compute_75,code=sm_75 -gencode=arch=compute_80,code=sm_80'
    echo -e '#!/bin/sh\nexport LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/opt/nccl/build/lib\nexport NCCL_PROTO=simple' | sudo tee /etc/profile.d/nccl.sh
